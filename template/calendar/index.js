...
const conf = {
  ...
  //   const ld = new Date(year, month, 0);
  //   const fd = new Date(year, month-1, 1);
  //   console.log(ld)
  //   console.log(fd)

  //   const lastDay = util.formatTime(ld);
  //   const firstDay = util.formatTime(fd);
  //   var that = this
  ...
  calculateDays(year, month, curDate) {
    ...
    days.forEach(item => {
      ...
      // 这里是加入关于打卡的信息的
      var td = new Date(item.year, item.month, item.day)
      var std = util.formatTime(td)    

      //console.log(tmplist)
      var signedList = tmplist
      //console.log(item)
      //console.log(signedList)
      for(var i = 0; i < signedList.length; i++){
        var d = util.formatTimeWx(signedList[i])
        if (item.year == d.getFullYear() && item.month == d.getMonth()+1 && item.day == d.getDate()){
          item.sign = true
          //console.log(item)
  ...
  choosePrevMonth() {
    ...
    const ld = new Date(newYear, newMonth, 0);
    const fd = new Date(newYear, newMonth - 1, 1);
    //console.log(ld)
    //console.log(fd)
    ...
    wx.request({
      ...
      success: function (res) {
        //console.log(res.data)
        tmplist = res.data
        conf.renderCalendar.call(that, newYear, newMonth);
      },
      fail: function (res) { 
        conf.renderCalendar.call(that, newYear, newMonth);
      },
  ...
  chooseNextMonth() {
    ...
    wx.request({
      ...
      fail: function (res) { 
        conf.renderCalendar.call(that, curYear, curMonth, curDate);
      },
      complete: function (res) { },
    })
  },
  ...
  jumpToToday() {
    ...
    const ld = new Date(curYear, curMonth, 0);
    const fd = new Date(curYear, curMonth - 1, 1);
    //console.log(ld)
    //console.log(fd)
